...
class ProfileController extends Controller
{
    ...
    public function update(ProfileUpdateRequest $request): RedirectResponse
    {
        ...
        $image_name = Auth::guard('web')->user()->profile_pic;
        if ($request->hasFile('profile_pic')) {
            ...
            if (Auth::guard('web')->user()->profile_pic != 'images/profile_pic/user/dummy.png') {
                File::delete(public_path(Auth::guard('web')->user()->profile_pic));
            }
        }

        BusinessUser::where('id', Auth::guard('web')->user()->id)->update([
            'name' => $request->name,
            'phone' => $request->phone,
            'email' => $request->email,
